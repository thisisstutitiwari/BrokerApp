...
const TenantPayRent = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8000/property/getbookedproperties/${userId}`)
      .then((propertyData) => {
        setPropertyData(propertyData.data.bookedProperties);
      })
      .catch((err) => {
        console.log(err);
      });
  }, [paidSuccess, paidError]);
  ...
  const handlePayRent = (propertyId) => {
    axios
      .post(`http://localhost:8000/property/payrent`, { propertyId, userId })
      .then((response) => {
        setPaidSuccess(response.data.message);
        setTimeout(() => {
          setPaidSuccess(null);
        }, 3000);
      })
      .catch((err) => {
        setPaidError("Failed to pay rent. Please try again.");
        console.log(err);
      });
  };
    ...
    <div className="col-12">
      ...
      >
        <div className="m-4 d-flex align-items-center justify-content-center">
          <h5 style={{ color: paidSuccess ? "green" : "red" }}>
            {paidSuccess ? paidSuccess : paidError ? paidError : ""}
          </h5>
        </div>
          ...
          propertyData.some((property) => property.status === "closed") ? (
            propertyData
              ...
              .map((property) => {
                ...
                let payStatusColor =
                  property.rentStatus.toLowerCase() === "unpaid"
                    ? "red"
                    : "green";
                    ...
                    >
                      ...
                      <div className="card-body">
                        <div className="d-flex justify-content-between align-items-center mb-3">
                          ...
                          <button
                            onClick={() => handlePayRent(property._id)}
                            ...
                            disabled={property.rentStatus === "paid"}
                          >
            ...
            <h2 className="my-5">No properties booked by you</h2>
          )}
        </div>
      </div>
    </div>
